...
export class CreateTableUser1724833301409 implements MigrationInterface {
    name = 'CreateTableUser1724833301409'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`user\` (\`id\` int NOT NULL AUTO_INCREMENT, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`username\` varchar(255) NOT NULL, \`password\` varchar(255) NOT NULL, \`fullName\` varchar(255) NOT NULL, \`phoneNumber\` varchar(255) NOT NULL, \`balance\` int NOT NULL DEFAULT '0', \`email\` varchar(255) NOT NULL, \`avatar\` varchar(255) NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`card\` DROP PRIMARY KEY`);
        await queryRunner.query(`ALTER TABLE \`card\` DROP COLUMN \`id\``);
        await queryRunner.query(`ALTER TABLE \`card\` ADD \`id\` int NOT NULL PRIMARY KEY AUTO_INCREMENT`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`card\` DROP COLUMN \`id\``);
        await queryRunner.query(`ALTER TABLE \`card\` ADD \`id\` varchar(36) NOT NULL`);
        await queryRunner.query(`ALTER TABLE \`card\` ADD PRIMARY KEY (\`id\`)`);
